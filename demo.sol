...
contract MsgRecall {
    ...
    // 可以通过txHash查询对应的事件
    event Recall(address from, address to, uint timestamp);
    ...
    mapping (address => mapping (address => uint)) Mapping;

    // 消息撤回记录上链接口，r,s,v表示用户授权签名
    function recall(address from, address to, uint timestamp, bytes32 r, bytes32 s, byte v) public {
        // 验证用户授权的签名
        // address addr = ecrecoverDecode(from, to, timestamp, r, s, v);
        //if (addr != from) revert(); // 如果验签不通过，则回滚

        // 记录from, to和timestamp
        Mapping[to][from] = timestamp;

        // 创建一个Recall事件，事件内容包含发起地址和对方地址
        emit Recall(from, to, timestamp);
    }
