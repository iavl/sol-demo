...
contract MsgRecall {
    ...
    // 可以通过txHash查询对应的事件
    event Recall(address from, address to, uint timestamp, int64 pk);
    ...
    function recall(address from, address to, uint timestamp, int64 pk, int64 r, int64 s) public {
        ...
        // 验证用户授权的签名
        // address addr = ecrecoverDecode(from, to, timestamp, r, s);
    ...
    // 使用ecrecover恢复出公钥
    function ecrecoverDecode(address from, address to, uint timestamp, int64 r, int64 s) public  returns (address addr) {
        ...
        // ecrecover(bytes32 hash, bytes32 r, bytes32 s) returns (address)
        // addr = ecrecover(hash, r, s);
    }

    // 查询接口
    function queryParams(address to, address from) public  returns (int64 pubkey, uint timestamp) {
